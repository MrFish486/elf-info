...
int main (int argc, char **argv){
	if (argc == 1) {
		...
		if (buffer[0x00] == 0x7f && buffer[0x01] == 0x45 && buffer[0x02] == 0x4c && buffer[0x03] == 0x46) {
			printf("File seems to be valid ELF file, reading...\n");
			printf("----------------------------------\n");
		}
		printf("Magic number                     : %02hx %02hx %02hx %02hx\n", buffer[0x00], buffer[0x01], buffer[0x02], buffer[0x03]);
		...
		printf("\n");
		// Flags
		printf("Flags (ISA dependent)            : ");
		if (address_space == 32) {
			printf("%02hx %02hx %02hx %02hx", buffer[0x24], buffer[0x25], buffer[0x26], buffer[0x27]);
		} else if (address_space == 64) {
			printf("%02hx %02hx %02hx %02hx", buffer[0x30], buffer[0x31], buffer[0x32], buffer[0x33]);
		}
		printf("\n");
		// Header size
		printf("ELF Header size                  : ");
		if (address_space == 32) {
			printf("%02hx %02hx", buffer[0x28], buffer[0x29]);
			printf(" (%d)", get_field(endianness, 2, buffer[0x28], buffer[0x29]));
		} else if (address_space == 64) {
			printf("%02hx %02hx", buffer[0x34], buffer[0x35]);
			printf(" (%d)", get_field(endianness, 2, buffer[0x34], buffer[0x35]));
		}
		printf("\n");
		// Program header table size
		printf("Program header table size        : ");
		if (address_space == 32) {
			printf("%02hx %02hx", buffer[0x2A], buffer[0x2B]);
			printf(" (%d: ", get_field(endianness, 2, buffer[0x2A], buffer[0x2B]));
			switch (get_field(endianness, 2, buffer[0x2A], buffer[0x2B])) {
				case 0x20:
					printf("32-bit, matches");
					break;
				case 0x38:
					printf("64-bit, unmatching");
					break;
				default:
			...
			printf("%02hx %02hx", buffer[0x36], buffer[0x37]);
			printf(" (%d: ", get_field(endianness, 2, buffer[0x36], buffer[0x37]));
			switch (get_field(endianness, 2, buffer[0x36], buffer[0x37])) {
				case 0x20:
					printf("32-bit, unmatching");
					break;
				case 0x38:
					printf("64-bit, matches");
					break;
				default:
					printf("non-standard size");
			}
		...
		printf("Program header table entry count : ");
		if (address_space == 32) {
			printf("%02hx %02hx", buffer[0x2C], buffer[0x2D]);
			printf(" (%d)", get_field(endianness, 2, buffer[0x2C], buffer[0x2D]));
		} else if (address_space == 64) {
			printf("%02hx %02hx", buffer[0x38], buffer[0x39]);
			printf(" (%d)", get_field(endianness, 2, buffer[0x38], buffer[0x39]));
		}
		printf("\n");
		// Section header entry size
		printf("Section header table entry size  : ");
		if (address_space == 32) {
			printf("%02hx %02hx", buffer[0x2E], buffer[0x2F]);
			printf(" (%d: ", get_field(endianness, 2, buffer[0x2E], buffer[0x2F]));
			switch (get_field(endianness, 2, buffer[0x2E], buffer[0x2F])) {
				case 0x28:
					printf("32-bit, matches");
					break;
				case 0x40:
			...
			printf("%02hx %02hx", buffer[0x3A], buffer[0x3B]);
			printf(" (%d: ", get_field(endianness, 2, buffer[0x3A], buffer[0x3B]));
			switch (get_field(endianness, 2, buffer[0x3A], buffer[0x3B])) {
				case 0x28:
					printf("32-bit, unmatching");
					break;
				case 0x40:
		...
		printf("Section header table entry count : ");
		if (address_space == 32) {
			printf("%02hx %02hx", buffer[0x30], buffer[0x31]);
			printf(" (%d)", get_field(endianness, 2, buffer[0x30], buffer[0x31]));
		} else if (address_space == 64) {
			printf("%02hx %02hx", buffer[0x3C], buffer[0x3D]);
			printf(" (%d)", get_field(endianness, 2, buffer[0x3C], buffer[0x3D]));
		}
		printf("\n");
		// Section header table name index
		printf("Section header table name index  : ");
		if (address_space == 32) {
			printf("%02hx %02hx", buffer[0x32], buffer[0x33]);
			printf(" (%d)", get_field(endianness, 2, buffer[0x32], buffer[0x33]));
		} else if (address_space == 64) {
			printf("%02hx %02hx", buffer[0x3E], buffer[0x3F]);
			printf(" (%d)", get_field(endianness, 2, buffer[0x3E], buffer[0x3F]));
		}
		printf("\n");
	}
}
